...
// ID of the last assistant message — used to correctly bind orphan artifacts
const lastAssistantMsgId = computed<string | undefined>(() =>
      ...
      >
        ...
        <template v-for="(msg, i) in chat.messages" :key="msg.id">
          ...
          <ToolCallBubble
            ...
            class="px-4"
          />
          <!-- Frozen tool calls after streaming ends -->
          <ToolCallBubble
            v-else-if="!chat.isStreaming && msg.role === 'assistant' && msg.toolCallRecords?.length"
          ...
          >
            <MessageBubble
              ...
              :artifacts="msg.role === 'assistant' ? chat.sessionArtifacts.filter(a => a.msgId ? a.msgId === msg.id : msg.id === lastAssistantMsgId) : undefined"
